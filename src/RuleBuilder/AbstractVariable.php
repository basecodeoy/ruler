...
abstract class AbstractVariable extends BaseVariable implements \ArrayAccess
{
    ...
    {
        $reflectionClass = new \ReflectionClass($this->ruleBuilder->findOperator($name));
        $args = \array_map([$this, 'asVariable'], $args);
        \array_unshift($args, $this);

        $op = $reflectionClass->newInstanceArgs($args);
    ...
    {
        $reflectionClass = new \ReflectionClass('\\BaseCodeOy\Ruler\\Operator\\'.$name);
        \array_unshift($args, $this);

        return $this->wrap($reflectionClass->newInstanceArgs($args));
    }

    /**
     * Private helper to wrap a VariableOperator in a Variable instance.
     */
    private function wrap(VariableOperator $variableOperator): self
    {
        return new static($this->ruleBuilder, null, $variableOperator);
    }
}
